...
const CardDog = ({ name, image, temperament, weight, createinDb, selectedTemperament }) => {
  const imageUrl = image ? image : DEFAULT_IMAGE_URL;

  // Check if the current temperament is selected
  const isSelected = selectedTemperament === temperament;

  return (
    <div className={`${s.master} ${isSelected ? s.selected : ""}`}>
      <img src={imageUrl} alt={name} width="300px" height="250px" />
      <div className={s.cardDatos}>
        <div className={s.contTitulo}>
        { typeof name === 'string' && name.length > 0 && (
  <h2 className={s.cardNombre}>name: {name.charAt(0).toUpperCase() + name.slice(1)}</h2>
)}
        </div>
        <div className={s.datos}>
          <h3 className={s.cardDato}>weight: {weight}</h3>
          <h4 className={`${s.temperamentos} ${isSelected ? s.selected : ''}`}>
      ...
      {createinDb && (
        <div className={s.cardDatos}>
          ...
          <div className={s.datos}>
            <h3 className={s.cardDato}>weight: {weight}</h3>
            <h4 className={s.temperamentos}>
              temperament: {temperament.map((temp) => temp).join(", ")}
            </h4>
