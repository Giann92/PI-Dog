import React, { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { getDetail } from '../../redux/actions';
import { Link } from 'react-router-dom';
import s from './detail.module.css';
import NavBar from '../../components/nav/NavBar';


// const DEFAULT_IMAGE_URL = 'data:image/png;base64,';

const Detail = (props) => {
  const dispatch = useDispatch();
  const details = useSelector((state) => state.detail);
  const [loading, setLoading] = useState(true); 
 
  console.log(details);
  
  useEffect(() => {
    dispatch(getDetail(props.match.params.id))
      .then(() => {
        setLoading(false); // Set loading to false after data is fetched
      });
  }, [dispatch, props.match.params.id]);

  if (loading) {
    return (
      <div>
        <img src="https://rosiephillpot.com/wp-content/uploads/2021/06/Karl_Sniffing_Export.gif" alt="Loading..."  width="400px" height="360px"/> {/* Display the loading GIF */}
        <p>Cargando los detalles del Perro...</p>
      </div>
    );
  }

  if (!details) {
    return <div>No se encontraron detalles para este Perro.</div>;
  }
  
  
  return (
    <>
    <NavBar/>
      <div className={s.dogDetail}>
        <h2 className={s.dogDetailName}>{details.name}</h2>
        <img className={s.dogDetail__image} src={details.image} alt={details.name}/>
        <h3 className={s.dogDetail__attribute}>Temperamento: {details.temperament}</h3>
        <h4 className={s.dogDetail__attribute}>Peso: {details.weight}</h4>
        <h4 className={s.dogDetail__attribute}>Altura: {details.height}</h4>
        <h4 className={s.dogDetail__attribute}>Años de vida: {details.life}</h4>
        <h4 className={s.dogDetail__attribute}> descripcion: {details.descripcion}</h4>
      </div>
        <div className={s.dogDetail__closeBtn}>
          <Link to="/dogs">Volver a la lista de Perros</Link>
        </div>
    </>
  );
};

export default Detail;

